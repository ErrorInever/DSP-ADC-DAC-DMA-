...
void DMA2_ADC_init(void) {
    ...
    // addresses
    DMA2_Stream0->PAR = (uint32_t)&(ADC1->DR);          // from ADC data register
    DMA2_Stream0->M0AR = (uint32_t)&adc_val;            // to
    DMA2_Stream0->NDTR = 1;                             // number of data 
    // setup CR
    DMA2_Stream0->CR = (0 << DMA_SxCR_CHSEL_Pos)  |     // channel 0 (for ADC1)
                        DMA_SxCR_MINC             |     // add increment (not necessarily)
                        DMA_SxCR_CIRC             |     // enable circular mode
                        (1 << DMA_SxCR_MSIZE_Pos) |     // Memory size: 16 bits (01)
                        (1 << DMA_SxCR_PSIZE_Pos);          // Peripheral size: 16 bits (01)
